...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6 p-4">
            <div
                class="rounded-3xl border border-sky-500/10 bg-gradient-to-br from-slate-950 via-slate-950 to-sky-950/50 p-6"
            >
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="space-y-2">
                        <p class="text-xs uppercase tracking-[0.35em] text-sky-300/80">Aurora Atlas</p>
                        <h1 class="text-2xl font-semibold text-slate-100">Calm signal flow</h1>
                        <p class="text-sm text-slate-400">
                            Gradient glows keep the focus on the headlines.
                        </p>
                    </div>
                    <div class="rounded-full border border-sky-400/30 bg-sky-400/10 px-4 py-2 text-xs text-sky-100">
                        Theme 05
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-[330px,1fr]">
                <div class="flex flex-col gap-6">
                    <Card class="border-sky-500/10 bg-slate-950">
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2 text-slate-100">
                                <Rss class="h-4 w-4 text-sky-300" />
                                Add a feed
                            </CardTitle>
                            ...
                            >
                                <div class="grid gap-2">
                                    ...
                                    <Input
                                        ...
                                        required
                                        class="border-sky-500/20 bg-slate-950 text-slate-100 placeholder:text-slate-500"
                                    />
                    ...
                    <Card class="border-sky-500/10 bg-slate-950">
                        ...
                        <CardContent class="space-y-4">
                            <div
                                v-if="feeds.length === 0"
                                class="rounded-lg border border-dashed border-sky-500/20 p-4 text-sm text-slate-400"
                            >
                            ...
                            <div
                                v-for="feed in feeds"
                                :key="feed.id"
                                class="rounded-2xl border border-sky-500/10 bg-slate-900/60 p-4"
                            >
                ...
                <Card class="flex h-full flex-col border-sky-500/10 bg-slate-950">
                    <CardHeader>
                    ...
                    <CardContent class="space-y-4">
                        <div
                            v-if="items.length === 0"
                            class="rounded-lg border border-dashed border-sky-500/20 p-6 text-sm text-slate-400"
                        >
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="space-y-1">
                                    <Link
                                        ...
                                        class="text-base font-semibold text-slate-100 transition-colors hover:text-sky-100"
                                    >
